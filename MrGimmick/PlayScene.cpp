...
void CPlayScene::Load()
{
	...
	CTextures::GetInstance()->Add(ID_TEX_BBOX3, L"textures\\bbox3.png", D3DCOLOR_XRGB(255, 255, 255));
...
void CPlayScene::Update(DWORD dt)
{
	...
	if (star->GetState() == STAR_SS_HIDE || star->GetState() == STAR_SS_PENDING || star->GetState() == STAR_SS_CREATED || star->GetState() == STAR_SS_READY) {
...
void CPlaySceneKeyHandler::OnKeyDown(int KeyCode)
{
	...
	if (gimmick->GetState() == GIMMICK_SS_DIE || gimmick->stunning == true)
		return;
...
void CPlaySceneKeyHandler::KeyState(BYTE* states)
{
	...
	if (gimmick->GetState() == GIMMICK_SS_DIE)
		return;

	// disable control key when Mario die 
	if (gimmick->GetState() == GIMMICK_SS_DIE) return;
	if (game->IsKeyDown(DIK_SPACE) && gimmick->stunning == false) {
		if (!gimmick->falling || gimmick->onInclinedBrick || gimmick->onEnemy || gimmick->jumping)
			gimmick->SetState(GIMMICK_SS_JUMP);
	}
	if (game->IsKeyDown(DIK_RIGHT) && gimmick->stunning == false)
		gimmick->SetState(GIMMICK_SS_WALKING_RIGHT);
	else if (game->IsKeyDown(DIK_LEFT) && gimmick->stunning == false)
		gimmick->SetState(GIMMICK_SS_WALKING_LEFT);
	else
		gimmick->SetState(GIMMICK_SS_IDLE);
}
