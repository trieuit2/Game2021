...
CGimmick::CGimmick() : CGameObject()
{
	untouchable = 0;
	stunning = false;
	SetState(GIMMICK_SS_IDLE);
}

void CGimmick::CalculateSpeed(DWORD dt) {
	if (this->state == GIMMICK_SS_IDLE) {
		if (vx > 0 && vx + ax * dt < 0) {
			vx = 0;
			ax = 0;
		}
...
void CGimmick::Update(DWORD dt, vector<LPGAMEOBJECT>* coObjects)
{
	...
	for (UINT i = 0; i < coObjects->size(); i++)
	{
		...
		if (dynamic_cast<CBlackEnemy*>(coObjects->at(i))) {
			...
			if (onTopOf(enemy, 7) && enemy->state == BLACKENEMY_SS_WALK && this->vy < 0 ) { 
				this->onGround = true;
				if(!onEnemy && !stunning) standOn(enemy); //fix loi khi cuoi nhieu quai 1 luc
			}
			if (isUnder(enemy)) { 
				this->SetState(GIMMICK_SS_STUN);
				StartUntouchable();
			}
		}
	...
	else
	{
		...
		for (UINT i = 0; i < coEventsResult.size(); i++)
		{
			...
			if (dynamic_cast<CBrickPink*>(e->obj)) {
				...
				{
					vy = BrickPink->vy;
					if (state == GIMMICK_SS_WALKING_RIGHT || state == GIMMICK_SS_WALKING_LEFT)
			...
			if (dynamic_cast<CConveyor*>(e->obj)) {
				...
				if (e->ny > 0)
				{
					...
					{
						if (state == GIMMICK_SS_WALKING_RIGHT)
						{
							x = x0 + min_tx * dx + 1.5f;
						}
						if (state == GIMMICK_SS_WALKING_LEFT)
						{
							x = x0 + min_tx * dx + 2.0f;
						}
						if (state == GIMMICK_SS_IDLE)
						{
							x = x0 + 2.0f;
						}
					}
			...
			if (dynamic_cast<CSwing*>(e->obj)) {
				...
				if (swing->GetState() == SWING_SS_OPEN || swing->GetState() == SWING_SS_STOP)
				{
					x += dx;
					y += dy;
				}
				else
				{
					if (e->ny > 0)
					{
						...
						{
							if (swing->GetState() == SWING_SS_STAND)
								swing->SetState(SWING_SS_MOVE);
						}
					}
			...
			if (dynamic_cast<CMedicine*>(e->obj))
			{
				...
				if (e->nx != 0)
				{
					medicine->SetState(MEDICINE_SS_DISAPPEAR);
				}
				if (e->ny != 0)
				{
					medicine->SetState(MEDICINE_SS_DISAPPEAR);
			...
			if (dynamic_cast<CBlackEnemy*>(e->obj)) {
				...
				if (e->ny > 0 && enemy->state == BLACKENEMY_SS_WALK) {
					vy = 0;
...
void CGimmick::Render()
{
	...
	if (this->stunning == true) {
		if (nx > 0)
			ani = GIMMICK_ANIMATION_STUN_RIGHT;
		else
			ani = GIMMICK_ANIMATION_STUN_LEFT;
		...
		if (vy > 0) {
			if (nx > 0)
				ani = GIMMICK_ANIMATION_JUMP_RIGHT;
			else
				ani = GIMMICK_ANIMATION_JUMP_LEFT;
		}
		else {
			if (vx == 0) 
			{ // IDLE AND FALL
				if (nx > 0) {
					if (this->onGround) ani = GIMMICK_ANIMATION_IDLE_RIGHT;
					else ani = GIMMICK_ANIMATION_JUMP_RIGHT;
				} 
				else {
					if (this->onGround) ani = GIMMICK_ANIMATION_IDLE_LEFT;
					else ani = GIMMICK_ANIMATION_JUMP_LEFT;
				}
			}
			else if (vx > 0)
			{ //WALK RIGHT
				if (this->onGround) ani = GIMMICK_ANIMATION_WALKING_RIGHT;
				else ani = GIMMICK_ANIMATION_JUMP_RIGHT;
			}
			else
			{ //WALK LEFT
				if (this->onGround) ani = GIMMICK_ANIMATION_WALKING_LEFT;
				else ani = GIMMICK_ANIMATION_JUMP_LEFT;
			}
		}
...
void CGimmick::CreateDieEffect() {
	...
	LPANIMATION_SET ani_set = animation_sets->Get(GIMMICKDIEEFFECT_ANIMATION_SET);
	for (int i = 0; i < 16; i++) {
